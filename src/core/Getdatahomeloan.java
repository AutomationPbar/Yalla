...
import pom.Elements3;
import utilities.DBManager;

public class Getdatahomeloan {

	WebDriver driver;
	WebDriverWait wait;
	String baseurl1 = "https://yallacompare.com/uae/en/mortgages/";
	...
	String tableName = "Automation.YallaHomeLoans";
	String ftext;
	String btext;
	...
	public void ccdata() {
		try {
			...
			int blistcount = 0;
				...
				Thread.sleep(2000);	
				Elements3.changebank(driver).click();
			...
			Elements3.changefeature(driver).click();
			Thread.sleep(1000);
			...
			Elements3.changefeature(driver).click();
				...
				try{
					...
					Thread.sleep(1000);
					ftext = Elements3.changefeature(driver).getText();
				...
				for(int ban=2;ban<=blistcount;ban++){
					...
					try{
						...
						Thread.sleep(1000);
						btext =Elements3.changebank(driver).getText();
						Thread.sleep(1000);
			...
			while (looper) {
				...
				for (int i = 1; i <=cccount; i++) {
					...
					try {
						...
						ccName = Elements3.creditcards(driver, i, 1).getText();

						System.out.println("credit card name :" + ccName);

						CCData.put("Card Name :", ccName);

						WebElement ccName1 = Elements3.creditcardsimage(driver, i, 1);
					...
					String NameURL="";
					try{
					NameURL = Elements3.moreinfo(driver,i).getAttribute("href");
					Thread.sleep(1000);
					}catch(Exception e){
						e.printStackTrace();
					}
					...
					if (handles.size() > 1) {
						...
						try{
							String ifin = Elements3.islamicfin(driver).getText();
							
							String cb = Elements3.cashback(driver).getText();
				...
				try {
					Actions builder = new Actions(driver);
				     builder.moveToElement(Elements3.nextpage(driver) ).click( Elements3.nextpage(driver) );
				     builder.perform();
