...
import pom.Elements4;
import utilities.DBManager;

public class Getdatacarloan {

	WebDriver driver;
	WebDriverWait wait;
	String baseurl1 = "https://yallacompare.com/uae/en/car-loans/";
	...
	String tableName = "Automation.YallaCarLoans";
	String ftext;
	String btext;
	...
	public void ccdata() {
		try {
			...
			int blistcount = 0;
				...
				Thread.sleep(2000);	
				Elements4.changebank(driver).click();
			...
			Elements4.changefeature(driver).click();
			Thread.sleep(1000);
			...
			Elements4.changefeature(driver).click();
				...
				try{
					...
					Thread.sleep(1000);
					ftext = Elements4.changefeature(driver).getText();
				...
				for(int ban=2;ban<=blistcount;ban++){
					...
					try{
						...
						Thread.sleep(1000);
						btext =Elements4.changebank(driver).getText();
						Thread.sleep(1000);
			...
			while (looper) {
				...
				for (int i = 1; i <=cccount; i++) {
					...
					try {
						...
						ccName = Elements4.creditcards(driver, i, 1).getText();

						System.out.println("credit card name :" + ccName);

						CCData.put("Card Name :", ccName);

						WebElement ccName1 = Elements4.creditcardsimage(driver, i, 1);
					...
					String Msal = Elements4.creditcards(driver, i, 2).getText();

					System.out.println("Minimum Salary :" + Msal);
					CCData.put("Minimum Salary :", Msal);

					String Mpay = Elements4.creditcards(driver, i, 3).getText();

					System.out.println("Monthly Payment :" + Mpay);

					CCData.put("Monthly Payment:", Mpay);

					String FRate = Elements4.creditcards(driver, i, 5).getText();

					System.out.println("Flat Rate  :" + FRate);

					CCData.put("Flat Rate :", FRate);
					
					String RRate = Elements4.creditcards(driver, i, 6).getText();

					System.out.println("Reducing Rate  :" + RRate);

					CCData.put("Reducing Rate :", RRate);

					String Stransfer = Elements4.creditcards(driver, i, 7).getText();
					...
					String NameURL="";
					try{
					NameURL = Elements4.moreinfo(driver,i).getAttribute("href");
					Thread.sleep(1000);
					}catch(Exception e){
						e.printStackTrace();
					}
					...
					if (handles.size() > 1) {
						...
						try{
							String ifin = Elements4.islamicfin(driver).getText();
							
							String cb = Elements4.cashback(driver).getText();
				...
				try {
					Actions builder = new Actions(driver);
				     builder.moveToElement(Elements4.nextpage(driver) ).click( Elements4.nextpage(driver) );
				     builder.perform();
